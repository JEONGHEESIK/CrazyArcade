...
void Bomb::debug(HDC hdc)
{
    ...
    PenWithHatchBrush(1, RGB(66, 110, 244), RGB(81, 243, 255), HS_BDIAGONAL)(hdc, [hdc, this]
        {
            drawRect(hdc, static_cast<int>(_start.x), static_cast<int>(_start.y), _SIZE, _SIZE);
        });

    Text(15, static_cast<int>(_start.x), static_cast<int>(_start.y), showBombStateForDebug(_currentState))(hdc);
}